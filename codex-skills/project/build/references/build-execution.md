...
section for any feature at `[~] planned, awaiting build` status (set by `$project-plan-feature`
...
> "No Gate 4-approved plans found. Run $project-plan-feature to create a feature plan first."
...
Update the feature plan using the Codex editing tools. Change the sub-feature's marker
from `[ ]` to `[x]`:
...
via the Codex shell tool with a reasonable timeout (default 120 seconds).
...
DIAGNOSIS: <Codex's analysis of why the test failed>
...
Use a concise user question with these 3 options (max 12-character headers: Fix,
Update, Exit).

**Fix:** Codex diagnoses and fixes the code to make the test pass. Re-runs the
...
**Update (BUILD-06, D-10):** The user provides a corrected test command. Codex
writes the new command to the feature plan's `## Test Command` section using the
Codex editing tools. Re-runs the updated command. If it passes, proceed. If it fails, show
the failure output and re-offer options.

**Exit:** Stop the session. Sub-features already marked `[x]` in the feature
plan are preserved. The user investigates manually. The next `$project-build` invocation
...
tracking is needed. The next `$project-build` invocation:
