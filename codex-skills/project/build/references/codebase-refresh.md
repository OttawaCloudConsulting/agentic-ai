...
**If no commit found** (first `$project-build` invocation after `$project-define`, or the
...
When files have changed, spawn a sub-agent using Codex sub-agent delegation to perform the
...
5. **Write the updated assessment** back to `docs/codebase-assessment.md` using
   the Codex filesystem editing tools.

### Sub-Agent Tool Access

The sub-agent uses: Codex file reads, Codex filesystem editing tools, Codex shell commands (for `ls`, `git` commands), and
file globbing tools.
...
> "No codebase assessment found. Run $project-define to create one."

This is a **warning, not a blocker**. `$project-build` can proceed without an
