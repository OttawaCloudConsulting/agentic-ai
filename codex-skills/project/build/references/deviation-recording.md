...
When Codex detects a deviation during sub-feature implementation:
...
Use a concise user question with two options:
...
- **Why necessary:** Explain the technical or practical reason the original
  approach could not be followed. This is the most important field for `$project-design`
  refresh mode.
...
Write the deviation entry to the feature plan using the Codex editing tools **as soon as
...
Use the Codex editing tools to append the deviation entry to the `## Architectural
...
> "Multiple deviations detected (N total). Consider running $project-design in refresh
