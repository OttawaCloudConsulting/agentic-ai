...
Gate 1 is the most complex gate in the `$project-define` skill. It covers the full PRD lifecycle: context loading, multi-round interview, document production, partial approval, checklist validation, gate approval recording, and revision mode for existing PRDs.
...
Conduct a multi-round interview using a concise user question. One round at a time -- never dump all questions at once. 2-4 questions per round. After each round, update `prd.md` and show the user what was added.
...
Maps to PRD sections: Milestones (captures initial scoping intent; detailed breakdown happens in `$project-milestone` after Gate 3)

**Interview rules:**

- One round at a time -- present 2-4 questions per round via a concise user question
- After each round, use the Codex editing tools to update `prd.md` and show the user what was added
...
Read `assets/prd-template.md` for the document structure. Use the Codex filesystem editing tools to create `prd.md` populated with the seed answers. Read the file back and confirm Summary and Goals sections are populated. Tell the user what was written and proceed to the interview rounds.

### Iterative Updates

After each interview round, use the Codex editing tools to update `prd.md` with new information. Show the user what was added before proceeding to the next round.
...
- Milestones section stays as "(to be defined)" -- populated later by `$project-milestone`
...
2. Use a concise user question with options: **Approve** / **Revise** / **Partial Approve**
3. If **Revise**: ask what needs changing, apply edits with the Codex editing tools, re-present the updated PRD (D-08)
...
2. Apply the requested changes using the Codex editing tools
...
4. Codex pre-checks `[x]` items that can be verified from the PRD content
...
Revision mode is triggered when `$project-define` detects an existing `prd.md` AND the user signals revision intent. This skips Gates 0 and WB entirely -- goes straight to Gate 1 revision.
...
5. **Update `prd.md`**: Apply revisions using the Codex editing tools. Show the user each change before and after.
