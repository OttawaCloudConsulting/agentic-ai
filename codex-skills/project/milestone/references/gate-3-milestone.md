...
Use a concise user question with options:
...
On Approve: update `prd.md` Milestones section using the Codex editing tools. Replace `(to be defined)` with the approved milestone plan formatted as a numbered list:
...
- **Sizing** -- confirm 2-5 features, note any large features that may need sub-feature splitting at `$project-plan-feature` time
...
1. **Gate 3 line (MIL-09):** If not already in-progress, update the Gate 3 line to:
   ...
   **Important:** `$project-milestone` NEVER writes `[x]` to the Gate 3 line. Only `$project` closes Gate 3 (D-05).
...
3. **Codex pre-checks** items that can be verified programmatically:
