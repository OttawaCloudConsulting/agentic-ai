...
Spawn a sub-agent to scan files relevant to the feature being planned. This is a targeted, feature-scoped scan -- narrower than `$project-design`'s 15-30 file architecture scan.
...
- **Sub-Features:** Break the feature into committable units of work. Each sub-feature should be completable within a single `$project-build` session (~120k tokens on a 200k model). See sizing validation below.
- **Interface Contracts:** Function/method signatures, data shapes (types/schemas), event formats. Enough detail for `$project-build` to implement without guessing interfaces (D-09).
- **Edge Cases:** Identify known edge cases from acceptance criteria, architecture constraints, and codebase scan
- **Test Command:** A single command to validate this feature. Codex proposes based on feature scope and existing test patterns; user confirms during review (D-08). Per DD-12, the user can adjust this mid-build without gate re-approval.
...
- **Architectural Deviations:** Default to `(none)`. This section is populated by `$project-build` during implementation, not by `$project-plan-feature`.
...
These are lighter than `$project-design`'s 2-4 callouts since plans are narrower in scope (single feature vs. full architecture).
...
3. **Use a concise user question** with options:
   - **Approve** -- plan is accepted, proceed to checklist validation
   - **Revise** -- plan needs changes

4. **If Revise:** Ask "What should change?" Apply edits to the plan file using the Codex editing tools. Re-present the revised plan. Repeat until the user selects **Approve**.

This is simpler than the section-by-section approval used in `$project-design` and `$project-milestone` because plans are single-feature scope.
...
Codex pre-checks items it can verify programmatically:
...
**CRITICAL: $project-plan-feature does NOT write to progress.txt.** Unlike `$project-milestone` which writes to both `milestone-status.txt` and `progress.txt`, `$project-plan-feature` writes ONLY to `milestone-status.txt`. Gate 4 is a feature-level event, not a milestone-level event. The `progress.txt` milestone summary line is only updated when features are completed by `$project-build`.

**Write-ordering (STATE-04):** Since `$project-plan-feature` writes only to `milestone-status.txt`, there is no multi-file write ordering concern. Write `milestone-status.txt` and stop.
...
- **NEVER write to progress.txt** -- $project-plan-feature writes ONLY to milestone-status.txt
