...
Revise ONLY affected sections using the Codex editing tools. Do NOT use the Codex filesystem editing tools to replace the entire file.
...
6. **Checklist validation** -- follow the same process as normal mode: Codex pre-checks programmatically verifiable items, presents remaining items to the user, all items must be `[x]` or `[-]` before re-approval.
