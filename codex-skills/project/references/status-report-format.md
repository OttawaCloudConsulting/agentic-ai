...
```
RECOMMENDED: Run `$project-plan-feature` to create the implementation plan for Feature 01.3: Password Reset
Also available:
  - `$project-spike` -- research a technical question
  - `$project-milestone` -- define the next milestone
```
...
RECOMMENDED: Run `$project-define` to begin project definition
```

This is the simplest possible report. No alternatives are shown because `$project-define` is the only valid next action.
