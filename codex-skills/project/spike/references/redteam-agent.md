...
The agent uses: Codex file reads, Codex shell commands, file globbing, rg or grep search, available web lookup tools.
...
5. **Separate from research context:** The red-team agent runs in its own sub-agent context, not as a continuation of the research agent's session. It forms its own impressions from reading the findings file, not from shared reasoning.
