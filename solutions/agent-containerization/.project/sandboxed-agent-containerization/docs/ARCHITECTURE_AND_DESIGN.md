...
- **Cascade warm-up.** Squid marks its `cache_peer` parents DEAD at start — on this topology the
  parents are its own loopback listeners — and revives them only when a request needs one, which
  costs that request a 500. The mediator therefore spends the sacrificial request itself at start
  (01.3 Deviation 10), so an agent's first call succeeds.
