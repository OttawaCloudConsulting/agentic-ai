...
| **`egress-mediator` container** | The single enforcement point and the only path to the internet (R1.2). Five roles in one process boundary: L7 CONNECT/SNI policy evaluation, authoritative DNS for the pod, audit writer, mTLS terminator for agent identity, and (gated on D6) upstream credential broker. Candidate implementations: **`iron-proxy`** (Go, Apache-2.0) or **Squid** in CONNECT-allowlist mode | Multi-homed onto `claude-net`, `codex-net`, `agy-net` and the external network. Exposes **exactly one port to each agent network** — the proxy listener — and no management or metrics port (see [Mediator hardening](#mediator-hardening) below). Reads the resolved egress policy; writes the audit stream to the sink. CA private key and broker secrets injected at runtime from a secret manager, never baked into the image and never on an agent-reachable volume |
...
The mediator holds brokered credentials for all three agents, the CA private key, the DNS
authority for the pod, and the audit log. It is the single component whose compromise yields
everything, and it therefore carries the same baseline as the agent containers plus three
additions:

- `cap_drop: ALL`, `no-new-privileges`, read-only root filesystem, non-root user (same as D15).
- Secrets and the CA private key injected at runtime from a secret manager — not in the image,
  not on a volume any agent can reach.
...
| **T28** TLS splice verification | Inspect the connection from inside each agent container; attempt to read plaintext at the mediator | No mediator CA is presented to any agent; the mediator holds no plaintext. Antigravity traffic is never intercepted | R5.15 (and R5.13) | Mediator |
...
| **Default MCP transport per agent and per server** | UNVERIFIED for all three agents | Determines whether any MCP traffic crosses the enforcement point at all. R7.15 requires the inventory to state it | Build |
