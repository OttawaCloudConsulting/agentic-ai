...
| Secret-mounted | Yes — the `claude` mTLS client key (Feature 01.6), the `github` profile's `github-token` Compose secret (Feature 02.3 SF-4), and the `kubernetes` profile's `kubeconfig` Compose secret (Feature 02.3 SF-5) |
...
a third (`kubeconfig`, Table A row S3, corrected below), delivered the same way but as a
`path_env` (`KUBECONFIG` points at the mounted secret's path, never the file content itself).
...
| S3 | `claude`, `codex`, `agy` | Secret-mounted | — | `kubeconfig`: dedicated, scoped kubeconfig (token or client-certificate auth only) | `/run/secrets/pack-kubernetes-kubeconfig`, mapped via `KUBECONFIG` to the mounted path — `path_env`, not `env` (Feature 02.3 SF-5, `packs/kubernetes/pack.yaml`) | Operator-set at the context credential's creation; no lifetime enforced by this architecture | Whatever RBAC scope the context's credential grants against the target cluster, for every agent under the `kubernetes` profile (Decision 1 — one kubeconfig, three containers, same rule that gives every agent a pack's egress). Unlike S1/S2, the credential is never read into the environment directly — `KUBECONFIG` names a file path, and the file itself is agent-readable from that path. The shipped pack grants no cluster reach (`egress.runtime: []`, Decision 6), so this credential is inert until a per-cluster egress pack is also loaded |
...
| S3 | **R8.5** — revocable at the cluster without an image rebuild (the kubeconfig is a Compose secret, never baked into a layer, R8.1). **R7.12** — independent of every other credential; revoking it does not touch V1–V3, E1–E4, S1 or S2. Scoped to one context (Decision 6) — a dedicated ServiceAccount token or client certificate, never the operator's own `~/.kube/config` identity | Revoke or rotate the context's credential at the cluster (delete the ServiceAccount token / client certificate). Rotation is a Compose secret-file swap plus `--force-recreate` — no rebuild | Same class of trigger as S2: any suspected exposure via the transcript sink (see R8.6 below) or a container compromise under the `kubernetes` profile | *(empty by design — 02.5)* |
...
| P4 | `claude`, `codex`, `agy` (`kubernetes` profile) | The `kubeconfig` never lands in the environment (it is `path_env`, not `env` — S3), so `printenv KUBECONFIG` returns only a path. But an echoed tool-call argument or command output that `cat`s the file, or a `kubectl config view --raw` invocation, puts the kubeconfig's own credential material — `client-key-data`, `client-certificate-data`, a bearer `token:` field, or a bare PEM block — into the faithful sink verbatim, the same class of exposure as P1–P3 one layer down (the file's content, not an env var) | **R8.6 (Feature 02.3 SF-5):** `redact_for_relay` extended with a kubeconfig-material family: `client-key-data:`/`client-certificate-data:` base64 fields, a bearer `token:` field, and PEM `-----BEGIN ... KEY-----`/`-----BEGIN CERTIFICATE-----` blocks. Asserted against synthetic transcript lines carrying each form — no live cluster or live session spent. Applies to the relay only, same as P1–P3; `$SINK_LOG` stays faithful |

**SF-5 redaction patterns (`recorder.sh`), extended at 02.3 SF-4 and SF-5.** Four families, applied
ONLY to the relayed copy, never to `$SINK_LOG`: proxy-URL userinfo (`://user:<redacted>@`, the P1
pattern above), known model-provider API-key/token prefixes (`sk-ant-`, `sk-proj-`, `sk-`, `AIza`),
GitHub token formats (`ghp_`, `github_pat_`, `gho_` — the P3 pattern above, added at SF-4), and
(added at SF-5) kubeconfig material — `client-key-data`/`client-certificate-data` fields, a bearer
`token:` field, and PEM key/certificate blocks (the P4 pattern above). Every redacted relay line
carries a `redacted` count (Decision 8). No prefix in any family has been observed in a live
transcript to date — all are a defense-in-depth floor, not a second measured finding, same posture
as the original SF-5 set.
