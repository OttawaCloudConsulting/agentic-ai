...
| V3 | `codex` | Volume-persisted | `oauth-mount` (**SF-4, not yet built**) | Same as V2 — the credential type is identical; only its provenance differs | Copied from the operator's host `~/.codex/auth.json` into `codex-state` during a one-shot bootstrap | Same as V2 | Same as V2 **plus a host-side consequence measured by SF-3: codex rolls its refresh token, so the container's first refresh supersedes the host's copy.** Treated conservatively as costing the operator their host codex login (R4.17). The host file also carries `OPENAI_API_KEY` alongside the OAuth set; the staged copy strips it, so E2's blast radius is not silently added to this row |
...
| V3 | As V2, plus the two structural controls SF-4 builds: the host source is mounted `:ro` (R4.13) as a **dedicated directory** rather than the credential file (R4.14), and **only during the one-shot bootstrap** — at steady state there is no host mount at all (R4.15), so an agent that deletes its own credential finds no source to re-copy from | As V2, **plus the host side**: the operator's own `~/.codex` session must be re-established, since the container's refresh supersedes the host's copy. Recorded as R4.17's accepted risk | As E1 | *(empty by design — 02.5)* |
